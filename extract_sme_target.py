...
# 关键词模式 (按优先级)
# 修正 P2-2026-07-06 (CEO 提供 URL 验证): 采购意向表格列名格式 "是否专门面向中小企业采购" + 行 "| ... | 是 | 否 |"
# 表格列名/值检测: 单独一类, 优先于正文关键词
# 严防误判: 表格列名出现不代表值是"是", 必须表格行里真的出现"是"
TARGETED_PATTERNS = [
    # 表格列名格式 (采购意向常见): "是否专门面向中小企业采购" + 表格行 "是"
    r'是否\s*专门\s*面向\s*中小[微小]?[\s\S]{0,500}?\|\s*是\s*\|',
    r'是否\s*专门\s*面向\s*小[微企业型][\s\S]{0,500}?\|\s*是\s*\|',
    # 正文格式 (招标公告常见)
    ...
    r'本项目\s*按照.*?专门\s*面向\s*中小[微小]?',
    # 最后才用宽松模式 (只匹配文中 "专门面向中小企业" 等明确表述, 不会命中表格列名)
    r'本项目\s*专门\s*面向\s*中小[微小]?\s*企业',
    r'本采购包\s*专门\s*面向\s*中小[微小]?\s*企业',
]

PREFERENCE_PATTERNS = [
    # 表格格式: "是否采购节能产品" / "价格优惠" 之类
    r'价格\s*扣除\s*[优惠折扣]?',
    ...
    r'小微[型企业].*?优惠',
    r'执行价格扣除优惠',
    # 表格列名格式 (采购意向): "价格扣除优惠政策" / "小微型企业扣除20%"
    r'非\s*预留份额[\s\S]{0,500}?价格\s*扣除\s*优惠',
]
