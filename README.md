...
## 数据质量（2026-07-08 v2.7）

### unified.db 四表

| 表 | 总数 | 发包方 | 金额 | 开标时间 |
|----|------|--------|------|--------|
| tender（招标公告） | 4048 | 90% | 81% | 90% |
| award（中标成交） | 4423 | 92% | 75% | — |
| intention（采购意向） | 1205 | 98% | 99% | — |
| other（流标/终止/更正/合同） | 3482 | — | — | — |

| 表/字段 | 覆盖率 | 说明 |
|---------|--------|------|
| award.winner | 87% | sufu 结构性0%（SPA 需登录） |
| project_links | 65% | tender×award 关联（2875条），project_chain 视图 |
...
scripts/utils/expand_intention.py       yancheng_gov 批次意向展开（表格→expected_list JSON）
...
scripts/legacy/   一次性 / 已用完的脚本（v2.6-v2.7 审计沉淀，保留为档案）
  reenrich_ycggzy_from_list_api.py   ycggzy 4050 条历史补救（已用完）
  reenrich_ycggzy.py                 ycggzy 698 条专项补采（已用完）
  reenrich_jszbcg_open_date.py       jszbcg openBidTime 1683 条修复（已用完）
  reenrich_yancheng_gov_from_page.py yancheng_gov from_page 补全（已用完）
  reenrich_yancheng_gov_p17.py       yancheng_gov p17 批次（已用完）
  fix_titles.py                      yancheng_gov 141 条乱码标题修复（已用完）
  fix_unique_index_scope.py          P0-1 UNIQUE INDEX 白名单迁移（幂等可重跑）
  cleanup_art_20171_dupes.py         art_20171 66 条脏数据清理（默认 dry-run）
  cleanup_orphan_dbs.py              0 字节孤儿 DB 自动清理
  migrate_from_old.py                从旧 history.db 迁移（已用完）

scripts/utils/    长期工具（产品级入口或可复用模块）
  expand_intention.py                yancheng_gov 批次意向展开（表格→expected_list JSON）
  migrate_unified_schema.py          unified.db 正式 schema 迁移（SCHEMA_MIGRATIONS 表驱动）
...
scripts/legacy/reenrich_ycggzy.py             ycggzy 专项补采（发包单位从列表 API 回填）
scripts/legacy/cleanup_art_20171_dupes.py     清理 yancheng_gov art_20171 脏数据（默认dry-run）
...
scripts/legacy/fix_titles.py                  修复 yancheng_gov 乱码标题（一次性，已用完）
scripts/legacy/migrate_from_old.py            从旧 history.db 迁移数据（一次性，已用完）
...
| 修复 | 新建 `scripts/legacy/reenrich_jszbcg_open_date.py`：读 page_path MD，正则解析 4 类格式（开标时间/文件开启时间/开启时间/开标日期）；支持 OCR 空格格式（2026 年 07 月 03 日 15 时 00 分） |
| 验证 | 候选 1710 条，命中 1690（99.4%），写入 DB 1683 条；合理性 99.9% open_date >= publish_date |
| 同步 | 移除月报/倒计时报告红字警告（字段已修复） |
| 文件 | `scripts/legacy/reenrich_jszbcg_open_date.py`（新增 156 行）、`generate_tender_report.py`（去警告）、`generate_countdown_report_pdf.py`（去警告） |
| git commit | `d1c46ab` |
...
| 修复 | ① 改 `crawlers/ycggzy.py:611` 改成 `json.dumps(item)` 保留 content ② 新建 `scripts/legacy/reenrich_ycggzy_from_list_api.py`（270 行）补救 4050 条历史 |
...
| 111 | yancheng_gov 批次意向公告展开（540条，59批次多项，表格→expected_list JSON）| `scripts/utils/expand_intention.py`（新增）|
...
| # | 位置 | 改动 | 验证 |
|---|------|------|------|
| 123 | `extract_sme_target.py`（新增 200 行） | 从 `data/pages/{site}/*.md` 缓存全文提取 3 类标签：`专门面向` / `非专门但优惠` / `不涉及`；排除「十、附件」模板段 | 抽 8 条人工核对准确率 100% |
| 124 | `data/unified.db` | `tender` + `intention` 表加 `sme_target` 列（TEXT） | 551 条 (13.8%) tender 有中小微政策（445 专门 + 106 优惠）|
| 125 | `generate_tender_report.py` | 清单表加列「中小微政策」专门面向项目用绿色●标记，优惠用橙色●，不涉及留空 | PDF 文本含「● 专门面向」标记 ✓ |
| 126 | `generate_intention_report.py` | 清单表加列「中小微」同样三色标记 | PDF 文本含「中小微」列 ✓ |
