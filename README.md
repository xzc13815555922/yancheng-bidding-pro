...
**当前版本**：v2.5 | **数据量**：8578 条（unified.db 三表合计） | **最后更新**：2026-06-25

> **v2.5 变更说明（2026-06-25）**：P0+P1 全站字段质量修复。① jszbcg budget 文件工本费/单价误提取清除（14条）② jszbcg winner "详见公告内容"伪值清除（19条）③ ycggzy purchaser &nbsp; HTML实体去除 ④ jszbcg tender budget 78%→85%（加"自筹资金"关键词）⑤ ycggzy CDATA修复新增1390条MD ⑥ yueda winner候选人格式 46%→94%。
...
## 数据质量（2026-06-25 v2.5）

### unified.db 三表

| 表 | 总数 | 发包方 | 金额 | 开标时间 |
|----|------|--------|------|--------|
| tender（招标公告） | 3728 | 90% | 85%↑(jszbcg) | 90% |
| award（中标成交） | 3857 | 92% | 74% | — |
| intention（采购意向） | 993 | 99%↑ | 95%↑ | — |

| 字段 | 说明 |
|------|------|
| purchaser（发包单位） | 整体 92%，v2.0~v2.5 累计清洗 200+ 条脏数据 |
| std_district（行政区划） | ~98% |
| std_category（项目分类） | 42%，规则持续扩充 |
| winner（中标人） | yueda 46%→94%↑（v2.5 候选人格式修复） |
...
data/
  *.db                    各站点独立 SQLite 数据库
  ...
  pages/{site}/           各站详情页本地 MD 缓存（~2900 个，v2.5 ycggzy+1390）
