...
| 125 | `generate_tender_report.py` | 第 1 页加「中小微企业专题（本月）」统计块（专门=绿/优惠=橙/不涉及=灰）；清单加列「中小微政策」专门面向项目用绿色●标记 | PDF 文本含「● 专门面向」标记 ✓ |
| 126 | `generate_intention_report.py` | 第 1 页加「中小微企业商机预警」说明，采购意向不写中小微（未出公告），但统计出口仍可追踪 | PDF 文本含「商机预警」字样 ✓ |
